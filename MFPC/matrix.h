...
inline float abs_t (float v) { return (float)fabs( v); } 
inline double abs_t (double v) { return fabs( v); } 
inline long double abs_t (long double v) { return fabsl( v); }
#endif
...
matrixT::pivot (size_t row)
{
  ...
  for (size_t i=row; i < _m->Row; i++)
    if ( (temp = abs_t( _m->Val[i][row])) > amax && temp != 0.0)
     {
       amax = temp;
       k = i;
     }
